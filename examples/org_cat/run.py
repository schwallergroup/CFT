...
def main():
    ...
    for i, a  in enumerate(cats_traj):
        ...
        #################################### this parts computes energy relative to probe infinitely far away
        ...
        # m.write_grid(f"cat_{i}_{m.atoms.info['smiles']}_conf_{m.atoms.info['conf_i']}.xyz")
        m.write_grid(f"tmp.xyz")
...
clean_calc = mace_mp(model=os.environ.get('MODEL_PATH', ''),  # set MODEL_PATH env var to your MACE model file
                device='cpu')
