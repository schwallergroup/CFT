    ...
    for trj_i, atoms in enumerate(cluster_trj):
        ...
        for f in fragments:
            m.make_fragment_population(
                population_size=population_size, fragment=f, coverage=coverage
            )
            ...
            for i in range(0, int(to_relax)):
                ...
                naked_relaxed_atoms = selected_atoms[
                    [atom.index for atom in selected_atoms if atom.symbol in metals]
                ]
                ...
                m.write_to_db(
                    surf_population_sorted + relaxed_trj + grids, db_path=db_file
                )
            exit()
...
prune_rms_thresh = 0.01  # default = 0.01
...
df = df.T
df = df[
    (df["energy_relative"] < 0.1) & df.element_symbol.isin(metals)
...
# fragments
dfl = pd.read_csv("data/phosphine_ligands_enriched.csv", delimiter=",")
dfl = dfl.dropna()
fragments = [
    Fragment(smi, to_initialize=f_conformers, prune_rms_thresh=prune_rms_thresh)
